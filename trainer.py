...
class Trainer():
    ...
    def train(self):
        self.scheduler.step()
        self.loss.step()
        epoch = self.scheduler.last_epoch + self.args.resume + 1
...
#            for i in range(4):
#                re_sr = torch.masked_select(sr[i], mask.to(device))
#                re_sr = re_sr.contiguous().view(N, C, outH, outW)
#                if i==0:
#                    loss = self.loss(re_sr, hr)
#                else:
#                    loss = loss + self.loss(re_sr, hr)
        ...
        with torch.no_grad():
            for idx_scale, scale in enumerate(self.scale):
                ...
                for idx_img, (lr, hr, filename, _) in enumerate(self.loader_test):
                    ...
                    sr = self.model(lr, idx_scale,scale_coord_map)
                    timer_test.hold()
                    re_sr = torch.masked_select(sr[-1],mask.to(device))
    ...
    def test1(self):
        test_scale = [1.5,2.0,2.5,3.0,3.5,4.0]
        epoch = self.scheduler.last_epoch + 1
        self.ckp.write_log('\nEvaluation:')
        self.ckp.add_log(torch.zeros(1, len(self.scale)))
        self.model.eval()
        ...
        device = torch.device('cpu' if self.args.cpu else 'cuda')
        with torch.no_grad():
            for idx_scale, scale in enumerate(test_scale):
                eval_acc = 0
                eval_acc_ssim = 0
                idx_scale1 = (idx_scale + 1) * 5 - 1
                self.loader_test.dataset.set_scale(idx_scale1)
                # tqdm_test = tqdm(self.loader_test, ncols=80)
                for idx_img, (lr, hr, filename, _) in enumerate(self.loader_test):
                    filename = filename[0]
                    no_eval = (hr.nelement() == 1)
                    ...
                    N, C, H, W = lr.size()
                    scale = test_scale[idx_scale]
                    outH, outW = int(H * scale), int(W * scale)
                    # _,_,outH,outW = hr.size()
                    # timer_test.tic()

                    scale_coord_map, mask = self.input_matrix_wpn(H, W, test_scale[idx_scale])
                    # position, mask = self.pos_matrix(H,W,self.args.scale[idx_scale])
                    # print(timer_test.toc())
                    if self.args.n_GPUs > 1 and not self.args.cpu:
                        scale_coord_map = torch.cat([scale_coord_map] * self.args.n_GPUs, 0)
                    ...
                    sr = self.model(lr, idx_scale1, scale_coord_map)
                    timer_test.hold()
                    re_sr = torch.masked_select(sr[-1], mask.to(device))
                    sr = re_sr.contiguous().view(N, C, outH, outW)
                    sr = utility.quantize(sr, self.args.rgb_range)
                ...
                self.ckp.log[-1, idx_scale1] = eval_acc / len(self.loader_test)
                best = self.ckp.log.max(0)
                # print(timer_test.acc/100)
                self.ckp.write_log(
                    '[{} x{}]\tPSNR: {:.3f} SSIM: {:.4f} (Best: {:.3f} @epoch {})'.format(
